package im.tox.tox4j.v2;

public final class ToxConstants {

    /**
     * The size of a Tox Client ID in bytes.
     */
    public static final int CLIENT_ID_SIZE		= 32;

    /**
     * The size of a Tox address in bytes. Tox addresses are in the format
     * [Client ID (TOX_CLIENT_ID_SIZE bytes)][nospam (4 bytes)][checksum (2 bytes)].
     *
     * The checksum is computed over the Client ID and the nospam value. The first
     * byte is an XOR of all the odd bytes, the second byte is an XOR of all the
     * even bytes of the Client ID and nospam.
     */
    public static final int ADDRESS_SIZE		= CLIENT_ID_SIZE + 4 + 2;

    /**
     * Maximum length of a nickname in bytes.
     */
    public static final int MAX_NAME_LENGTH		= 128;

    /**
     * Maximum length of a status message in bytes.
     */
    public static final int MAX_STATUS_MESSAGE_LENGTH	= 1007;

    /**
     * Maximum length of a friend request message in bytes.
     */
    public static final int MAX_FRIEND_REQUEST_LENGTH	= 1016;

    /**
     * Maximum length of a single message after which it should be split.
     */
    public static final int MAX_MESSAGE_LENGTH		= 1368;

    /**
     * Maximum size of custom packets. TODO: should be LENGTH?
     */
    public static final int MAX_CUSTOM_PACKET_SIZE	= 1373;

    /**
     * The number of bytes in a hash generated by tox_hash.
     */
    public static final int HASH_LENGTH                = /*crypto_hash_sha256_BYTES*/ 32;

}
