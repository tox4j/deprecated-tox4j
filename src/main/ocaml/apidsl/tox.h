...
namespace file {
  ...
  enum KIND {
    ...
    DATA,
    /**
     * Avatar data. This consists of $hash(image) + image.
     *
     ...
     * sending ${CONTROL.RESUME}). The avatar file name will contain the
     * hash in binary (not human-readable). A client can compare this hash with a
     * saved hash and send ${CONTROL.CANCEL} to terminate the avatar
