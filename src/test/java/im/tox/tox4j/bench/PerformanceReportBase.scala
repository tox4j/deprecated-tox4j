...
abstract class PerformanceReportBase extends PerformanceTest.OfflineRegressionReport {
  ...
  protected val toxInstance = Gen.single("tox")(classOf[ToxCoreImpl]).map(_ => ToxCoreFactory.make(new ToxOptions))
  ...
  protected val iterations = Gen.range("iterations")(500000, 1500000, 100000)
