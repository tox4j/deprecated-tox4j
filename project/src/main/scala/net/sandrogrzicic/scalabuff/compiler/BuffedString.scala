...
class BuffedString(str: String) {
   ...
   */
  def toScalaIdent = {
    lowerCamelCase match {
      case id @ ("type" | "match" | "def" | "object" | "class" | "trait" | "val" | "var") =>
        s"`$id`"
      case id =>
        id
    }
   ...
   */
  def lowerCamelCase = BuffedString.camelCaseRegex.replaceAllIn(str.replace('-', '_'), _.matched.tail.toUpperCase)
